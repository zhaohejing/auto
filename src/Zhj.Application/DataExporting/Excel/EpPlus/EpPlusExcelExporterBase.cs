...
namespace MyCompanyName.AbpZeroTemplate.DataExporting.Excel.EpPlus
{
    public abstract class EpPlusExcelExporterBase : AbpZeroTemplateServiceBase, ITransientDependency
    {
        ...
        protected void AddFooter(ExcelWorksheet sheet, int rowIndex,int col, string Value) {
            sheet.Cells[rowIndex, col].Value = Value;
            sheet.Cells[rowIndex, col].Style.Font.Bold = true;
        }
        protected void AddFooters(ExcelWorksheet sheet, int rowIndex, params string[] decitext) {
            if (decitext.IsNullOrEmpty()) {
                return;
            }

            for (var i = 0; i < decitext.Length; i++) {
                AddFooter(sheet, rowIndex, i + 1, decitext[i]);
            }
