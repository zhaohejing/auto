...
namespace MyCompanyName.AbpZeroTemplate.Authorization.Users {
    public class UserAppService : AbpZeroTemplateAppServiceBase, IUserAppService {
        ...
        public PagedResultOutput<UsersDto> GetUserList(GetUsersInput input) {

            var list = _sqlExecuter.GetUserList<UsersDto>(input.Filter).ToList()
                .WhereIf(!input.Filter.IsNullOrWhiteSpace(),
                c=>c.Name.Contains(input.Filter));

            var totalcount = list.Count( );
            var a = list.OrderBy(input.Sorting).Skip(input.SkipCount).Take(input.MaxResultCount).ToList( );


            return new PagedResultOutput<UsersDto>(totalcount, a);
        }
