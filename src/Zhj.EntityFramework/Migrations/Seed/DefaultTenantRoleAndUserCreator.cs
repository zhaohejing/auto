    ...
    public class DefaultTenantRoleAndUserCreator {
        ...
        private void CreateDefaultTenantAndUsers() {
            ...
            if (adminRoleForDefaultTenant == null) {
                adminRoleForDefaultTenant = _context.Roles.Add(new Role(defaultTenant.Id, StaticRoleNames.Tenants.Admin, "系统管理员") { IsStatic = true });
                _context.SaveChanges();
            }
            ...
            if (userRoleForDefaultTenant == null) {
                _context.Roles.Add(new Role(defaultTenant.Id, StaticRoleNames.Tenants.User, "操作员") { IsStatic = true, IsDefault = true });
                _context.SaveChanges();
            }
            ...
            if (adminUserForDefaultTenant == null) {
                adminUserForDefaultTenant = User.CreateTenantAdminUser(defaultTenant.Id, "admin", "admin@defaulttenant.com", "1234567");
                ...
                #region 添加操作员
                //var defaultUser = _context.Users.FirstOrDefault(u => u.TenantId == defaultTenant.Id && u.UserName == "操作员");
                //if (defaultUser == null) {
                //    defaultUser = User.CreateTenantAdminUser(defaultTenant.Id, "操作员", "admin@defaulttenant.com", "1234567");
                //    defaultUser.IsEmailConfirmed = true;
                //    defaultUser.IsActive = true;
                //    _context.Users.Add(defaultUser);
                //    _context.SaveChanges();

                //    //Assign Admin role to admin user
                //    _context.UserRoles.Add(new UserRole(defaultUser.Id, userRoleForDefaultTenant.Id));
                //    _context.SaveChanges();

                //    foreach (var permission in permissions) {
                //        if (!permission.IsGrantedByDefault) {
                //            if (!permission.Name.Contains("Pages.OrganizationUser")) {
                //                _context.Permissions.Add(
                //                   new RolePermissionSetting {
                //                       Name = permission.Name,
                //                       IsGranted = true,
                //                       RoleId = userRoleForDefaultTenant.Id
                //                   });
                //            }
